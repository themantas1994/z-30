...
def _init(run_oracle, rx_kwargs, arm_b):
    import z30
...
def _decode(job):
    ...
    if _W["arm_b"] is not None:
        rep_b = _W["rx"].decode_slot(window.tobytes(), **{**_W["rx_kwargs"], **_W["arm_b"]})
        out["b"] = any(d["payload"] == payload for d in rep_b["decodes"])
        out["b_false"] = sum(1 for d in rep_b["decodes"] if d["payload"] != payload)
    return out
...
def main():
    ...
    ap.add_argument("--arm-b", default=None,
                    help="JSON overrides for a second vNext arm on the same buffers (paired A/B of a receiver change)")
    ...
    arm_b = json.loads(args.arm_b) if args.arm_b else None
    ...
    rows = []
    t_start = time.time()
    with Pool(args.workers, initializer=_init, initargs=(run_oracle, rx_kwargs, arm_b)) as pool:
        for snr in snrs:
            ...
            if arm_b is not None:
                row.update({"b": sum(r["b"] for r in res), "b_false": sum(r["b_false"] for r in res),
                            "only_a": sum(r["vnext"] and not r["b"] for r in res),
                            "only_b": sum(r["b"] and not r["vnext"] for r in res)})
            rows.append(row)
            ...
            if arm_b is not None:
                line += f"  arm B {row['b']:4d} (A-only {row['only_a']}, B-only {row['only_b']})"
    ...
    if arm_b is not None:
        a = sum(r["only_a"] for r in rows)
        b = sum(r["only_b"] for r in rows)
        p = mcnemar_exact_p(a, b)
        summary.update({"arm_b": arm_b, "ab_only_a": a, "ab_only_b": b, "ab_mcnemar_p": p})
        print(f"arm A (defaults{' + ' + json.dumps(rx_kwargs) if rx_kwargs else ''}) vs arm B {json.dumps(arm_b)}: "
              f"A-only {a}, B-only {b}; exact two-sided McNemar p = {p:.3g}")
    if args.out:
