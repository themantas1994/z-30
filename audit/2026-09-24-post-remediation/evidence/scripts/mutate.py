...
W = os.path.join(S, "mut")
ENV = dict(os.environ, CARGO_TARGET_DIR=os.path.join(S, "target_mut"), CARGO_TERM_COLOR="never")
...
def main():
    ...
    json.dump({"baseline": {" | ".join(" ".join(t) for t in g): ok for g, ok in base_ok.items()}, "mutations": results},
              open(os.path.join(S, "mutation_results.json"), "w"), indent=2)
