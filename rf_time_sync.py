...
class WWVDecoder(BaseStationDecoder):
    ...
    def process_dwell_stream(
        ...
        for i in range(0, len(envelope_1khz) - win_samples, step_samples):
            ...
            if chunk_energy > max_1khz_energy and chunk_energy > avg_env * 1.6:
        ...
        # Fail if no 800ms pulse found above noise floor or SNR too low
        if max_1khz_energy == 0.0 or snr_db < 4.5:
            return None
...
class DCF77Decoder(BaseStationDecoder):
    ...
    def process_dwell_stream(
        ...
        mod_depth = (max_val - min_val) / max_val if max_val > 1e-4 else 0.0
        ...
        if edge_idx == -1 or mod_depth < 0.35:
            return None
