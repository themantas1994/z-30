...
const FILES = [
  ...
  ['z30_dsp/ap_decode.py', 'A priori decoding: the QSO-state hypothesis ladder ported from WSJT-X, the gates that keep it narrow, and the constrained decode the CRC-14 arbitrates.'],
  ['z30_dsp/message_codec.py', '28-bit callsign packing and the payload field layout - the Python twin of the callsign half of src/dsp/z30Codec.ts.'],
  ...
  ['z30_dsp/station_settings.py', 'Station configuration schema, callsign and Maidenhead validation, and JSON persistence - importable without Tkinter so the validation rules can be tested.'],
  ...
  ['z30_dsp/git_sync.py', 'Upstream synchronisation: how many commits behind origin/main this installation is, and a fast-forward-only update that refuses to overwrite local work.'],
  ['z30_dsp/updater.py', 'Terminal front end for git_sync - the same engine the Update button in the web UI drives.'],
  ...
  ['tests/test_config_wizard.py', 'Callsign and grid validation, driven by the same shared vectors the TypeScript transmit gate is asserted against.'],
  ['tests/test_ap_decode.py', 'A priori decoding tests: that a pinned bit survives every iteration, that a wrong hypothesis is always rejected, and that an AP-less decode is unchanged.'],
