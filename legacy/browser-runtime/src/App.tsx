...
export default function App() {
  ...
  // No network request at startup (audit 2026-09-24, M-04): the unconditional GitHub update
  // check that ran here 2.5 s after every launch, with no opt-out, is removed. The update modal
  // still checks when the operator asks it to.
  ...
  const assertCanTransmit = useCallback(
    (audioOffsetHz: number, messageText?: string): boolean => {
      const permission = rigctl.canTransmit(config, audioOffsetHz, dialFreqHz, messageText);
  ...
  const startActiveTransmission = useCallback(async () => {
    ...
    // The whole message goes through the gate, not just the station (audit 2026-09-24, C-06).
    if (!assertCanTransmit(qsoEngine.getState().txFreqHz, qsoEngine.getCurrentTxMessage(config))) return;
    if (isTuning) {
