...
export interface DecodedSignal {
  ...
  // There is no per-decode confidence: the decoder has no calibrated one. The field used to be
  // the constant 99 on every decode, AP-assisted ones included (audit 2026-09-24, M-03).
  /**
   * Where the decode came from. 'synthetic-self-test' frames were generated from known symbols
   * by the Experimental self-test at a CONFIGURED SNR and DT = 0; they are not receptions.
   */
  source?: 'live-audio' | 'synthetic-self-test';
  rawSymbols?: number[];
...
export interface LogEntry {
  ...
  rstSent: string;
  rstRcvd: string;
  // Absent unless both grids were actually known (audit 2026-09-24, C-05: the auto-logger
  // computed a distance from the default grid EM00 when none was received).
  distanceKm?: number;
  azimuthDeg?: number;
  txPowerWatts?: number;
  myCall?: string;
