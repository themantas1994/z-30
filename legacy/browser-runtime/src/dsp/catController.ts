...
import { decodeCallsign28, encodeCallsign28, packZ30Message, unpackZ30Message } from './z30Codec';
...
export class CatController {
  ...
  public canTransmit(
    ...
    // 0. The frame itself. The packer is lossy: a grid outside its table goes out as another
    //    grid (FN42 as RE78), a partner call it cannot carry as another station (ZY2ABC as
    //    24BWE), a roger report as a plain one (R-12 as -12). Checking only our own callsign
    //    (step 1) let all of those through (audit 2026-09-24, C-06). A message that does not
    //    unpack to exactly the text requested is refused, whatever part of it changed.
    if (messageText !== undefined) {
      const wanted = messageText.trim().toUpperCase().replace(/\s+/g, ' ');
      let sent: string | null = null;
      try {
        sent = unpackZ30Message(packZ30Message(wanted).infoBits).rawText;
      } catch {
        sent = null;
      }
      if (sent !== wanted) {
        violations.push(
          sent === null
            ? `"${wanted}" cannot be encoded as a z-30 v1 message.`
            : `"${wanted}" would be transmitted as "${sent}": z-30 v1 cannot carry it exactly, so it is not sent.`
        );
      }
    }
  ...
  public async executeRawCommand(input: string, txContext?: RawConsoleTransmitContext): Promise<string> {
    ...
    if (verb === 'f' || long === '\\get_freq') {
      ...
      // This console never asked the radio. It used to answer 'VFOA' whatever the radio was
      // doing (audit 2026-09-24, M-01 / F-04); RPRT -11 is Hamlib's "not available".
      resp = 'RPRT -11 (not available: this console does not read the VFO from the radio)';
      status = 'ERROR';
    } else if (verb === 's' || long === '\\get_split_vfo') {
      resp = `${this.splitState ? 1 : 0}\n${this.txFreqHz}\n(this software's commanded split state, not read from the radio)`;
    } else if (verb === 'l' || long === '\\get_level') {
      resp = `${Math.round(this.getSmeterDb())}\n(estimated from the received AUDIO level, not the radio's S-meter)`;
    } else if (long === 'dump_state' || long === '\\dump_state') {
      // Used to answer "rig_model=3073 mfg_name=Icom status=READY" with any radio or none.
      resp = 'RPRT -11 (not available: this console has no rig state to dump; run rigctl against rigctld directly)';
      status = 'ERROR';
    } else if (long === 'version' || long === '\\version') {
      // 'v' deliberately NOT accepted here: it is \get_vfo above, and claiming it made this
      // branch dead code.
      resp = `z-30 bundled catalogue of Hamlib ${CURRENT_HAMLIB_VERSION.version} (not the version of any running rigctld)`;
