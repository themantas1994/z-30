...
export function toDecodedSignal(
  ...
  // A self-test frame carries its label in the text itself, so no view of the decode list can
  // show it as a reception (audit 2026-09-24, H-03).
  const message = frame.synthetic ? `SYNTHETIC TEST: ${frame.unpacked.rawText}` : frame.unpacked.rawText;
  return {
    ...
    source: frame.synthetic ? 'synthetic-self-test' : 'live-audio',
...
export class Z30SicDecoderEngine {
  ...
  public runSicDecodeCycle(
    ...
    // Settings -> Experimental Testing. Each is synthesised from its KNOWN symbols at its known
    // frequency, demodulated with a sigma computed from its CONFIGURED SNR, and reported with
    // that configured SNR and DT = 0 - so it exercises demodulation and LDPC, not acquisition,
    // and its SNR/DT are not measurements. Every such frame is marked synthetic and labelled
    // SYNTHETIC TEST in the decode list (audit 2026-09-24, H-03).
    // ==========================================================================
