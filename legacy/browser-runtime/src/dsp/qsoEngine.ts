    ...
    if (targetedMsg && config.autoSeq) {
      ...
      if (this.state.stage === 'CALLING_CQ' || this.state.stage === 'IDLE') {
        ...
        if (targetedMsg.message.includes('RRR') || targetedMsg.message.includes('73') || targetedMsg.message.includes('RR73')) {
          ...
          // Create Log Entry - only what was received, sent or measured (audit 2026-09-24,
          // C-05). This used to write local time labelled UTC (toTimeString), the grid FN31 and
          // the report -16 when none had been received, and a distance from the grid EM00.
          const dxGrid = this.state.targetDxGrid || '';
          const geom = dxGrid && config.myGrid ? calculateMaidenheadDistanceAndAzimuth(config.myGrid, dxGrid) : null;
          const now = new Date();
          const logEntry: LogEntry = {
            ...
            rstRcvd: targetedMsg.report || '',
            ...(geom ? { distanceKm: geom.distanceKm, azimuthDeg: geom.azimuthDeg } : {}),
