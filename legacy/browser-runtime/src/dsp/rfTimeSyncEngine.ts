 ...
 */
/** RF time synchronisation is retired: it never decoded a time code (audit C-03). */
export const RF_TIME_SYNC_RETIRED = true;
export const RF_TIME_SYNC_RETIRED_REASON =
  'RF time synchronisation has been removed: this engine never decoded a time code and could ' +
  'report success on noise (audit 2026-09-24, C-03). No offset was measured or stored. Keep the ' +
  'computer clock on UTC with NTP or GPS; z-30 uses the operating system clock.';
...
export class RfTimeSyncEngine {
  ...
  public demodulateTimeSignal(
    ...
    // RETIRED (2026-09-24 audit, C-03). None of the demodulators below decodes a time code:
    // the "RF timestamp" was derived from the system clock being measured, SNR was floored
    // (Math.max(snrDb, 6.0 / 7.2 / 8.5)), confidence was clamped to >= 0.70 or a constant, and
    // pure noise could produce "success". An offset from here was persisted and could step the
    // OS clock. This entry point now always fails, so nothing can report or persist a
    // synchronisation that did not happen. The code below is kept only as the record of what was
    // wrong; it is unreachable. z-30 (the Rust application in crates/) uses the OS clock.
    if (RF_TIME_SYNC_RETIRED) {
      return {
        success: false,
        ...
        jitterMs: 0,
        confidence: 0,
        details: RF_TIME_SYNC_RETIRED_REASON,
        envelopeCurve: [],
...
/**
 * RETIRED network time query (audit 2026-09-24, F-09 / M-05). It called one HTTP round trip
 * "atomic" and "sub-millisecond", reported offsets to 0.01 ms, and parsed timeapi.io's
 * zone-less `dateTime` as LOCAL time - an error equal to the machine's UTC offset. It now
 * always fails without contacting anything. Use the operating system's NTP client.
 */
  ...
  public static async queryAtomicUtcOffset(): Promise<{
    ...
    if (RF_TIME_SYNC_RETIRED) {
      return {
        ...
        rttMs: 0,
        serverTimeUtc: '',
        error: 'Network time query removed (audit 2026-09-24, M-05): use the operating system NTP client.',
      };
    }
    const endpoints = [
