...
class Z30AudioEngine {
  ...
  public injectTestSignal(
    // The 'SIC_COLLISION' preset was removed (audit 2026-09-24, H-03): each of its two signals
    // was demodulated from its own clean replica, so they never interfered and the "collision"
    // always "recovered" - a demonstration of SIC that involved no SIC.
    presetOrMessage: 'S9_CQ_JA1ABC' | 'S9_PLUS_G4XYZ' | 'WEAK_VK3XYZ' | 'CUSTOM' | string,
    options?: {
      freqHz?: number;
      snrDb?: number;
      playAudio?: boolean;
