...

{
  ...
  // The console never asks the radio for its VFO or its state, so it must not answer as if it
  // had: it used to reply 'VFOA' and "rig_model=3073 mfg_name=Icom status=READY" with any radio
  // or none (audit 2026-09-24, M-01).
  check('v says the VFO is not available rather than inventing one', (await rig.executeRawCommand('v')).startsWith('RPRT -11'));
  check('dump_state invents no rig state', (await rig.executeRawCommand('dump_state')).startsWith('RPRT -11'));
  check(
    'version names the bundled catalogue, not a running Hamlib, and is not shadowed by v',
    (await rig.executeRawCommand('version')).includes('bundled catalogue'),
    await rig.executeRawCommand('version')
  );
  check('\\version works too', (await rig.executeRawCommand('\\version')).includes('bundled catalogue'));
...
// ------------- C-06: the whole message must round-trip, not only the station's own callsign
group('C-06: a message the packer would change is refused, whatever part of it changes');
{
  const rig = new CatController();
  const station = { ...DEFAULT_STATION_CONFIG, regulatoryRegion: 'US', licenseClass: 'US_GENERAL', myCall: 'K1ABC' };
  // Audit 2026-09-24 E029: each of these went out as something else.
  for (const text of ['CQ K1ABC FN42', 'CQ K1ABC JO01', 'ZY2ABC K1ABC -10', 'G4XYZ/P K1ABC -10', 'G4XYZ K1ABC R-12', 'G4XYZ K1ABC -31']) {
    const p = rig.canTransmit(station, 1500, 14_076_000, text);
    check(`"${text}" is refused`, p.allowed === false && p.violations.some((v) => v.includes('cannot')), JSON.stringify(p.violations));
  }
  for (const text of ['CQ K1ABC FN31', 'G4XYZ K1ABC -10', 'G4XYZ K1ABC RR73', 'G4XYZ K1ABC 73']) {
    const p = rig.canTransmit(station, 1500, 14_076_000, text);
    check(`"${text}" round-trips and is allowed`, p.allowed === true, JSON.stringify(p.violations));
  }
}
