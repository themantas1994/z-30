...
-------------------------------------------------------------------
THIS IS THE FROZEN ORACLE'S REFERENCE RECEIVER, NOT THE PRODUCTION RECEIVER. z-30's receiver is
`decode_slot` in crates/z30-dsp; its published figures come from `z30 --benchmark suite`. The
figures below describe this reference receiver, in simulation, and nothing else.

`--mode realistic` (default) measures a **decode threshold** of the reference receiver. Every
frame gets a random carrier offset (+-5 Hz), a random timing offset (+-0.5 s) and optionally
Watterson HF fading; the receiver estimates its own noise level and searches for the frame
itself (`z30_dsp.acquisition`) - but only within +-12 Hz and +-0.55 s of the KNOWN nominal
carrier and slot start, so its acquisition is partly assisted (2026-09-24 audit, section 32;
measured there to make no difference against a whole-band search). It is comparable in
definition, not in conditions, with other modes' published simulation figures.
...
Quoting that figure beside another mode's published threshold - FT8's -21 dB, say, a
simulation figure from Franke, Somerville & Taylor (QEX 2020) for a receiver that finds the
signal itself - compares two different quantities and flatters this one. Measured on this code at seed DEFAULT_BENCHMARK_SEED, 200
