...
def apply_watterson_fading(
    ...
    Gaussian noise with the corresponding Gaussian impulse response, then normalised to unit
    ENSEMBLE power: averaged over realisations the channel neither adds nor removes power, so
    the SNR the caller asked for is the average SNR the receiver sees - while any one frame
    keeps the power its fading realisation gives it.
