...
# The oracle and the retired browser runtime both live under legacy/ now; the shared vectors
# stay at the repository root, where the Rust tests read them too.
ORACLE_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
REPO_ROOT = os.path.abspath(os.path.join(ORACLE_ROOT, "..", ".."))
TS_ROOT = os.path.join(REPO_ROOT, "legacy", "browser-runtime")
VECTOR_DIR = os.path.join(REPO_ROOT, "tests", "vectors")
TS_CONSTANTS = os.path.join(TS_ROOT, "src", "dsp", "z30Constants.ts")
TS_CODEC = os.path.join(TS_ROOT, "src", "dsp", "ldpcCodec.ts")
TS_WAVEFORM = os.path.join(TS_ROOT, "src", "dsp", "z30Waveform.ts")
VECTORS = os.path.join(VECTOR_DIR, "crc14_vectors.json")
TS_AP = os.path.join(TS_ROOT, "src", "dsp", "apDecode.ts")
CALLSIGN_PACK_VECTORS = os.path.join(VECTOR_DIR, "callsign_pack_vectors.json")
...
def test_crc_constants_match():
    ...
    py_source = read(os.path.join(ORACLE_ROOT, "z30_dsp", "ldpc.py"))
...
def test_sic_candidate_detection_constants_match():
    ...
    """
    ts_source = read(os.path.join(TS_ROOT, "src", "dsp", "realReceiver.ts"))
...
DITHER_VECTORS = os.path.join(VECTOR_DIR, "dither_vectors.json")
...
def test_no_alpha_constructor_argument_survives():
    ...
    engine = read(os.path.join(TS_ROOT, "src", "dsp", "monteCarloEngine.ts"))
...
def test_the_benchmark_demodulates_like_the_receiver_that_ships():
    ...
    """
    receiver = read(os.path.join(TS_ROOT, "src", "dsp", "realReceiver.ts"))
...
def test_the_browser_engine_uses_the_shipped_decoder_iteration_cap():
    ...
    """
    engine = read(os.path.join(TS_ROOT, "src", "dsp", "monteCarloEngine.ts"))
    modal = read(os.path.join(TS_ROOT, "src", "components", "MonteCarloBenchmarkModal.tsx"))
...
def test_the_python_sources_stay_importable_on_the_supported_floor():
    ...
    sources = [
        (directory, name)
        for directory in (os.path.join(ORACLE_ROOT, "z30_dsp"), os.path.join(ORACLE_ROOT, "tests"))
...
def test_ap_stage_ladder_matches():
    ...
    types_source = read(os.path.join(TS_ROOT, "src", "types", "z30.ts"))
...
def test_ap_closing_modifiers_match_the_packers_codes():
    ...
    codec_source = read(os.path.join(TS_ROOT, "src", "dsp", "z30Codec.ts"))
